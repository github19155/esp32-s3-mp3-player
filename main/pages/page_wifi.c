...
static void do_scan(void)
{
    ...
    s_ap_count = (int)num;
    ESP_LOGI(TAG, "Scan: %d AP(s)", s_ap_count);
}
...
static void roller_mask_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_COVER_CHECK) {
        ...
        rect.y1 = rect.y2 + font_h + line_space - 1;
        rect.y2 = coords.y2;
...
static void list_item_cb(lv_event_t *e)
{
    ...
    for (int i = 0; i < s_ap_count; i++) {
        ...
        if (strcmp(text, info) == 0) {
            ESP_LOGI(TAG, "Connect to: %s", s_ap_records[i].ssid);
            show_password_input(i);
            return;
        }
    }
}
